...
void writeSolution (char* fileName, uint32_t* dists, uint32_t v) {
  uint32_t i, j;
  ...
  fprintf (f, "All pair shortest distances\n");
  for (i = 0; i < v; ++i) {
      for (j = 0; j < v; ++j) {
          if (dists[i*v+j] > INFINITY_LIKE) fprintf(f, "%7s", "inf");
          else fprintf (f, "%7u", dists[i*v+j]);
      }
      fprintf(f, "\n");
  }
  fclose(f);
...
int main (int argc, char* argv[]) {
    ...
    if (argc < 3) {
        printf("Missing input and output filename.\n");
        exit(1);
    }
